...
$string = $_GET['string']; // shortened url characters
$method = $_GET['method']; // remove by url or by shortened value
...
// *******************************************************************	
	...
	if ($method == "url") {
		$query = "DELETE FROM urls WHERE url = '{$string}'";
	}
	
	if ($method == "short") {
		$query = "DELETE FROM urls WHERE chars = '{$string}'";
	}


	$database->query($query, SQLITE_BOTH, $error);
	echo "<br>Error:{$error}";
